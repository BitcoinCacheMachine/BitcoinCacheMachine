...
4) Deploy essential BCM data center components by running `bcm provision`. The components deployed by `bcm provision` are common to ALL BCM deployments and include TOR services (SOCKS5 proxy, TOR-enabled DNS, & TOR Control), docker registries, and a kafka logging stack. (WORK IN PROGRESS) The BCM SDN Controller intelligently deploys components across failure domains (i.e., individual x86_64) to achieve local high-availability.
