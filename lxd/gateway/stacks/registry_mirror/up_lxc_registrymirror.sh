...
if [[ -d $DIR ]]; then
    ...
    lxc file push $DIR/$CERT_CN.cert $BCM_LXC_GATEWAY_CONTAINER_NAME/apps/$LXC_STACK/server.cert
    lxc file push ~/.bcm/runtime/$LXC_REMOTE/$LXC_HOST/$LXC_STACK/$CERT_CN.key $BCM_LXC_GATEWAY_CONTAINER_NAME/apps/$LXC_STACK/server.key
    lxc file push ~/.bcm/certs/rootca.cert $BCM_LXC_GATEWAY_CONTAINER_NAME/apps/$LXC_STACK/ca.crt

    lxc exec $BCM_LXC_GATEWAY_CONTAINER_NAME -- chown root:root /apps/$LXC_STACK/server.cert
    lxc exec $BCM_LXC_GATEWAY_CONTAINER_NAME -- chown root:root /apps/$LXC_STACK/server.key
