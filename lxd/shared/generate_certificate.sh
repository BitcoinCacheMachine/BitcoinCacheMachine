...
if [ ! -f ~/.bcm/runtime/$LXC_REMOTE/$LXC_HOST/$LXC_STACK/$LXC_STACK.cert ]; then
    ...
    openssl req -new -newkey rsa:4096 -sha256 -days 365 -nodes -x509 -subj "/C=US/ST=BCM/L=INTERNET/O=BCM/CN=bcm" -keyout ~/.bcm/runtime/$LXC_REMOTE/$LXC_HOST/$LXC_STACK/$LXC_STACK.key -out ~/.bcm/runtime/$LXC_REMOTE/$LXC_HOST/$LXC_STACK/$LXC_STACK.cert
