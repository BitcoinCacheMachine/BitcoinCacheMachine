...
lxc file push -p -r ./build/ "$BCM_MANAGER_HOST_NAME"/root/gateway/

IMAGE_NAME="$BCM_PRIVATE_REGISTRY/bcm-docker-base:$BCM_DOCKER_BASE_TAG"

lxc exec "$BCM_MANAGER_HOST_NAME" -- docker build --build-arg BCM_DOCKER_BASE_TAG="$BCM_DOCKER_BASE_TAG" -t "$IMAGE_NAME" /root/gateway/build/
