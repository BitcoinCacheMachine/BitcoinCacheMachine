...
#         lxc exec "$BCM_BITCOIN_HOST_NAME" -- docker exec -t "$DOCKER_CONTAINER_ID" bitcoin-cli "$CHAIN_TEXT" "$CMD_TEXT" | jq
#     else
...
#             lxc exec "$BCM_BITCOIN_HOST_NAME" -- docker exec -it "$DOCKER_CONTAINER_ID" lncli --network="$BCM_ACTIVE_CHAIN" "$CMD_TEXT"
#         else
#             lxc exec "$BCM_BITCOIN_HOST_NAME" -- docker exec -t "$DOCKER_CONTAINER_ID" lncli --network="$BCM_ACTIVE_CHAIN" "$CMD_TEXT" | jq
#         fi
#     else
