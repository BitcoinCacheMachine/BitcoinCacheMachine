...
if [[ $BCM_CLI_VERB == "provision" ]]; then
    ...
    bcm-trezor:$BCM_VERSION trezor-agent -c $SSH_USERNAME@$SSH_HOSTNAME -- 'set -e && echo "BCM_GIT_DIR:  $BCM_GIT_DIR" && echo "deb https://deb.torproject.org/torproject.org bionic main" | sudo tee -a /etc/apt/sources.list && echo "deb-src https://deb.torproject.org/torproject.org bionic main" | sudo tee -a /etc/apt/sources.list && curl https://deb.torproject.org/torproject.org/A3C4F0F979CAA22CDBA8F512EE8CBC9E886DDD89.asc | sudo gpg --import && gpg --export A3C4F0F979CAA22CDBA8F512EE8CBC9E886DDD89 | sudo apt-key add - && sudo apt-get update && sudo apt-get install -y tor curl wait-for-it git deb.torproject.org-keyring && wait-for-it -t 30 127.0.0.1:9050 && git config --global "http.$BCM_GITHUB_REPO_URL.proxy" socks5://127.0.0.1:9050 && mkdir -p "$HOME/.bcm" && if [[ ! -d "$HOME/.bcm/.git" ]]; then git clone --quiet --single-branch --branch dev "$BCM_GITHUB_REPO_URL" "$HOME/.bcm"; fi && cd "$HOME/.bcm" && git fetch && git checkout dev && git pull && bash -c "$$HOME/.bcm/bcm" && echo "Restarting SSH endpoint." && sudo shutdown -r now'
fi
