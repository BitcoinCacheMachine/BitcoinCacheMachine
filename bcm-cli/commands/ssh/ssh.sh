...
if [[ -z $BCM_TREZOR_SSH_AUTHORIZED_KEYS_DIR ]]; then
    echo "BCM_TREZOR_SSH_AUTHORIZED_KEYS_DIR is empty. Setting to ~/.bcm/projects/bcm-dev/trezor/ssh"
    export BCM_TREZOR_SSH_AUTHORIZED_KEYS_DIR=~/.bcm/projects/bcm-dev/trezor/ssh
fi
