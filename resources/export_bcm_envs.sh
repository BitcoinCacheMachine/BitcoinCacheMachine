...
BCM_LXD_CLUSTER_NAME=""

if [[ $(lxc remote get-default) = "local" ]]; then
  BCM_LXD_CLUSTER_NAME="DEV_MACHINE"
else
  BCM_LXD_CLUSTER_NAME=$(lxc info | grep server_name | awk 'NF>1{print $NF}')
fi

if [[ -z $BCM_LXD_CLUSTER_NAME ]]; then
  echo "BCM_LXD_CLUSTER_NAME is not defined. Current value is '$BCM_LXD_CLUSTER_NAME'"
  exit
fi

# export some variables yo
export BCM_LXD_CLUSTER_NAME=$BCM_LXD_CLUSTER_NAME
export BCM_MULTIPASS_VM_NAME=$(lxc remote get-default)
export BCM_CLUSTER_ROOT_DIR=~/.bcm/clusters/$BCM_LXD_CLUSTER_NAME
